...
public class MGTP5Ex3App {
	...
	private static void createCurves(IEntity rootEntity) throws OperationNotSupportedException {
		...
		ICurve curve = new CurveSqrtBezierMulti(curveEntity);
		curve.getControlPts().add(new Vec3(-2f, -2f, 0f));
		
		curve.getControlPts().add(new Vec3(-1f, 0f, 0f));
		curve.getControlPts().add(new Vec3(0f, 1f, 0f));
		
		curve.getControlPts().add(new Vec3(1f, 1f, 0f));
		
		curve.getControlPts().add(new Vec3(2f, 1f, 0f));
		curve.getControlPts().add(new Vec3(3f, 0f, 0f));
