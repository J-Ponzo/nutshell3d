...
public class MGTPVolEx3App {
	...
	private static float seuil = 0.6f;
					...
					if (voxGrid[i][j][k] > seuil ) {
						if (i == 0 || j == 0 || k == 0 || i == w - 1 || j == h - 1 || k == d - 1 
								|| !(voxGrid[i + 1][j][k] > seuil 
										&& voxGrid[i - 1][j][k] > seuil 
										&& voxGrid[i][j + 1][k] > seuil 
										&& voxGrid[i][j - 1][k] > seuil 
										&& voxGrid[i][j][k + 1] > seuil 
										&& voxGrid[i][j][k - 1] > seuil)) {
			...
			for (int j = 0; j < h; j++) {
				for (int k = 0; k < d; k++) {
					...
					for (Sphere sphere : spheres) {
						...
						voxGrid[i][j][k] += (float) (sphere.getRadius() * a * Math.exp(-b * dist));
					}
				}
			}
		}
	...
	private static void configureScene() throws OperationNotSupportedException {
		...
		//Create spheres
		Sphere s1 = new Sphere(new Vec3(5.0f, 0.0f, 0.0f), 0.75f);
		spheres.add(s1);
		Sphere s2 = new Sphere(new Vec3(-5.0f, 0.0f, 0.0f), 0.75f);
		spheres.add(s2);
		Sphere s3 = new Sphere(new Vec3(0.0f, 5.0f, 0.0f), 0.75f);
		spheres.add(s3);
		Sphere s4 = new Sphere(new Vec3(0.0f, -5.0f, 0.0f), 0.75f);
		spheres.add(s4);
