...
public class MultiCamApp {
	...
	private static void createDrone(IEntity rootEntity) throws OperationNotSupportedException {
		ITransform transform;

		//Create Drone
		IEntity droneEntity = new Entity();
		transform = new Transform(droneEntity);
		transform.setLocalTranslate(Matrices.translation(0f, 0.5f, -3f));
		new AbstractUpdator(droneEntity) {
		...
		ICamera cameraFwd = new Camera(cameraEntity);
		cameraFwd.setWidth(width);
		cameraFwd.setHeight(height);
		cameraFwd.setNear(1);
		cameraFwd.setFar(100);
		cameraFwd.setFov(60f);
		cameraFwd.setViewport(
				new Rectangle(50, 0, 50, 50)
				);
		cameraFwd.setOrtho(false);
		cameraFwd.setMaterial(camMat);
		droneEntity.addChild(cameraEntity);

		//Create Bck Camera
		cameraEntity = new Entity();
		transform = new Transform(cameraEntity);
		transform.setLocalRotate(Matrices.yRotation((float) (Math.PI)));
		ICamera cameraBck = new Camera(cameraEntity);
		cameraBck.setWidth(width);
		cameraBck.setHeight(height);
		cameraBck.setNear(1);
		cameraBck.setFar(100);
		cameraBck.setFov(60f);
		cameraBck.setViewport(
				new Rectangle(50, 50, 50, 50)
				);
		cameraBck.setOrtho(false);
		cameraBck.setMaterial(camMat);
		droneEntity.addChild(cameraEntity);
		...
		ICamera cameraLeft = new Camera(cameraEntity);
		cameraLeft.setWidth(width);
		cameraLeft.setHeight(height);
		cameraLeft.setNear(1);
		cameraLeft.setFar(100);
		cameraLeft.setFov(60f);
		cameraLeft.setViewport(
				new Rectangle(0, 0, 50, 50)
				);
		cameraLeft.setOrtho(false);
		cameraLeft.setMaterial(camMat);
		droneEntity.addChild(cameraEntity);
		...
		transform = new Transform(cameraEntity);
		transform.setLocalRotate(Matrices.yRotation((float) (-Math.PI / 2)));
		ICamera cameraRight = new Camera(cameraEntity);
		cameraRight.setWidth(width);
		cameraRight.setHeight(height);
		cameraRight.setNear(1);
		cameraRight.setFar(100);
		cameraRight.setFov(60f);
		cameraRight.setViewport(
				new Rectangle(0, 50, 50, 50)
				);
		cameraRight.setOrtho(false);
		cameraRight.setMaterial(camMat);
		droneEntity.addChild(cameraEntity);

		SceneManager.getInstance().setActiveCamera(cameraRight);
	}
